import { IOrderMessage, IRatingTypes, IReviewMessageDetails, ISellerDocument } from '@eoladapo/jobman-shared';
...
const updateSellerOngoingJobProp = async (sellerId: string, ongoingJobs: number): Promise<void> => {
  await SellerModel.updateOne({ _id: sellerId }, { $inc: { ongoingJobs } }).exec();
};

const updateSellerCancelledJobProp = async (sellerId: string): Promise<void> => {
...
const updateSellerCompletedJobs = async (data: IOrderMessage): Promise<void> => {
  ...
  await SellerModel.updateOne(
    { _id: sellerId },
    { $inc: { ongoingJobs, completedJobs, totalEarnings }, $set: { recentDelivery: new Date(recentDelivery!) } }
  ).exec();
};
