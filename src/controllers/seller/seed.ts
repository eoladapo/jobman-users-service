...
import { BadRequestError, IBuyerDocument, IEducation, IExperience, ISellerDocument } from '@eoladapo/jobman-shared';
...
const seller = async (req: Request, res: Response): Promise<void> => {
  ...
  for (let i = 0; i < buyers.length; i++) {
    ...
    if (checkIfSellerExist) {
      throw new BadRequestError('Seller already exist', 'Seed seller() method error');
    }
  ...
  res.status(StatusCodes.CREATED).json({ message: 'Seed seller successfully' });
};
